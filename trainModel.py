...
def modelforID(interval,iden,timesteps):
	...
	model = createModel(10,64,10,24,data.shape[1:],2,interval,iden,timesteps) # input shape must be samples,number of time steps in each sample,number of features in each timestep
	model.train(values,labels,0.05,1,70)
