...
class createModel():
	...
	def __init__(self, dense, denseUnits ,inputShape, interval, timesteps,learning_rate,dropout):#add learning rate maybe? 
		...
		#if you want to import and continue training and old model uncomment and put the rest of the code under the else statement
		...
		#metrics will always be the same for binary classification problems so i didnt include them as parameters
...
#func that uses updater to get data and passes it to create model object that trains model using data. 
def modelforID(interval,iden,timesteps):
	...
	model = createModel(3,32,data.shape[1:],interval,timesteps,0.00005,0.1) #these parameters work best in my testing
	model.train(data,labels,0.1,5,2000)

if __name__ == "__main__":
	modelforID("24h",[22481,22324,12924,22486,20997,22978,8921,4151,13263],7)